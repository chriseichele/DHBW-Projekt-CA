...
$cn = $csr->common_name;
  ...
  if($type === 'Apache2'){
    $path = "c:\apache24\ca\kunden\\temp_".date("Y-m-d-H-i-s")."_".$cn.".crt";
    file_put_contents($path, file_get_contents($pathToCRT).PHP_EOL);
    file_put_contents($path, file_get_contents("c:\Apache24\ca\ica.crt").PHP_EOL , FILE_APPEND);
    file_put_contents($path, file_get_contents("c:\Apache24\ca\ca.crt").PHP_EOL , FILE_APPEND);
    return $path;
  }
    elseif($type === 'nginx'){
      $path = "c:\apache24\ca\kunden\\temp_".date("Y-m-d-H-i-s")."_".$cn.".chained.crt";
      file_put_contents($path, file_get_contents($pathToCRT).PHP_EOL);
      file_put_contents($path, file_get_contents("c:\Apache24\ca\ica.crt") , FILE_APPEND);
      file_put_contents($path, file_get_contents("c:\Apache24\ca\ca.crt").PHP_EOL , FILE_APPEND);
      return $path;
    }
      elseif($type === 'generic'){
        #generic Code
        $path = "c:\apache24\ca\kunden\\temp_".date("Y-m-d-H-i-s")."_".$cn.".zip";
        $zip = new ZipArchive();
